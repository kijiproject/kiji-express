...
class KijiJobSuite extends KijiSuite {
  ...
  test("A KijiJob can run with a pipe that uses packAvro.") {
    ...
    def validatePacking(outputBuffer: Buffer[GenericRecord]) {
      val inputMap = packingInput.toMap
      outputBuffer.foreach { r =>
        assert(inputMap(r.get("l").asInstanceOf[String]) === r.get("s"))
        assert(r.get("o") === "default-value")
      }
    }
    ...
    class PackTupleJob(args: Args) extends KijiJob(args) {
      TextLine(args("input")).read
          ...
          .packGenericRecordTo(('line, 'length) -> 'record)(SimpleRecord.getClassSchema)
